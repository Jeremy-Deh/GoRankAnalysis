...
######################################
# Fonction de création du model ML
######################################

def get_models(modelLGBM,modelLineaire,modelCatBoost):
    model_stack = dict()
    model_stack['lgbm']=modelLGBM.fit(X_train,y_train)
    model_stack['lineaire']=modelLineaire.fit(X_train,y_train)
    model_stack['cat1']=modelCatBoost.fit(X_train,y_train)
    model_stack['stacking'] = get_stacking(modelLGBM,modelLineaire,modelCatBoost).fit(X_train,y_train)
    return model_stack

def get_stacking(modelLGBM,modelLineaire,modelCatBoost):
    # define the base models
    level0 = list()
    level0.append(('lgbm', modelLGBM))
    level0.append(('lineaire', modelLineaire))
    level0.append(('catboost', modelCatBoost))
...
######################################
#Fonction Katrain analyse partie
######################################
def interp_ix(lst, x):
    i = 0
...
def game_report(game, thresholds, depth_filter=None):
    ...
    for n in nodes:
        ...
        filtered_cands_list.append(filtered_cands)
        #print("JD -",n,"cands :",filtered_cands,"\n\n")
        weight = min(
            1.0,
    ...
    return sum_stats, histogram, player_ptloss,filtered_cands
...
for sgf in os.listdir("sgftest/"):
    if sgf.lower().endswith("sgf"):
        ...
        partie=Game(katrain, engine, move_tree=move_tree, analyze_fast=True)
        games.append(partie)
    n += 1
...
cands_list=[]
for game in games:
    sum_stats, histogramJD, player_ptlossJD, filtered_cands= game_report(game, thresholds=thresholds)
    ...
    whiteTop1=sum_stats["W"]["ai_top_move"]
    whiteTop5=sum_stats["W"]["ai_top5_move"]
    whiteAccuracy=sum_stats["W"]["accuracy"]
    whiteComplexity=sum_stats["W"]["complexity"]
    blackTop1=sum_stats["B"]["ai_top_move"]
    blackTop5=sum_stats["B"]["ai_top5_move"]
    blackAccuracy=sum_stats["B"]["accuracy"]
    blackComplexity=sum_stats["B"]["complexity"]
     
    percentileBlack=np.percentile(black, np.arange(5, 101, 5)).tolist()
    percentileWhite=np.percentile(white, np.arange(5, 101, 5)).tolist()
    percentile=np.arange(5, 101, 5).tolist()
    percentile=["p"+str(x) for x in percentile]
    index=["Pseudo","RangJ","RangAdv","Moyenne","Mediane","Top1","Top5","Accuracy","Complexity"]
    index.extend(percentile)

    
    
    dataNoir=[game.root.properties["PB"],rankingChiffre(game.root.properties["BR"]),rankingChiffre(game.root.properties["WR"]),black.mean(),black.median(),blackTop1,blackTop5,blackAccuracy,blackComplexity]
    dataNoir.extend(percentileBlack)
    dataBlanc=[game.root.properties["PW"],rankingChiffre(game.root.properties["WR"]),rankingChiffre(game.root.properties["BR"]),white.mean(),white.median(),whiteTop1,whiteTop5,whiteAccuracy,whiteComplexity]
    dataBlanc.extend(percentileWhite)
    
    noir=pd.Series(dataNoir,index=index)
    blanc=pd.Series(dataBlanc,index=index)
...
df4.to_csv("AnalyseAvance.csv")



df4.drop(df4[df4.RangJ<=-30].index,inplace=True)
...
X=df4.drop(["Unnamed: 0","RangJ","Pseudo","RangAdv"],axis=1)
y=df4.RangJ
X_train, X_test, y_train, y_test = train_test_split(X,y, test_size=0.2)
...
clf1=lgb.LGBMRegressor(num_leaves=30,boosting_type="dart")
clf2=LinearRegression()
clf3=CatBoostRegressor(verbose=0,learning_rate=0.2,iterations=100)
mstack = get_models(clf1,clf2,clf3)

for i in mstack.keys() :
    print(i)
    mstack[i].fit(X_train, y_train)
    y_pred=mstack[i].predict(X_test)
    print("Jeu de test : ",mstack[i].score(X_test,y_test))
    print("Jeu de train : ",mstack[i].score(X_train,y_train))
    print()

print(r2_score(y_test,y_pred))
df=pd.concat([pd.Series(y_test,name="Rang reel").reset_index(drop=True),pd.Series(y_pred,name="Rang predit").reset_index(drop=True)],axis=1)
df


with open('modelTrained.pkl', 'wb') as f1:
    pickle.dump(mstack['stacking'], f1)
    
with open('modelResult.txt', 'w') as f:
    for i in mstack.keys() :
        f.write(str(i)+"\n")
        mstack[i].fit(X_train, y_train)
        y_pred=mstack[i].predict(X_test)
        f.write("Jeu de test : "+str(mstack[i].score(X_test,y_test))+"\n")
        f.write("Jeu de train : "+str(mstack[i].score(X_train,y_train))+"\n")

    somme=0
    for i in np.arange(-15,10,1):
        if i>0:
            niveau=str(i)+"d"
        else:
            niveau= str(abs(i)+1)+"k"
        f.write(str(niveau)+":"+str(sum(df4.RangJ==i))+"\n")
        somme=somme+sum(df4.RangJ==i)
    f.write(str(somme))